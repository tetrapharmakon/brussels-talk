...
\input{layout/L-packages.tex}
\input{layout/L-commands.tex}
\input{layout/L-alphabet.tex}
...
\begin{frame}
	% 	\begin{itemize}
	% 		\item Formal category theory studies an \alert{ambient 2-category} $\mathcal K$
	% 		\item and makes some assumptions on  $\mathcal K$ ensuring that it is possible to interpret category theory in it
	% 		\item there are many competing standards to achieve this:
	% 		      \begin{itemize}
	% 			      \item Yoneda structures
	% 			      \item proarrow equipments
	% 		      \end{itemize}
	% 	\end{itemize}
	% \end{frame}
	% \begin{frame}
	% 	\centering\Huge The state of the art
	% \end{frame}
	% \begin{frame}
	% 	The notion of Yoneda structures recognizes the Yoneda lemma as the core of category theory.

	% 	Axioms are given in order to interpret the presheaf construction in $\mathcal K$, and in particular the ``fibrational Yoneda lemma''
	% 	\[
	% 		p^\leftarrow c \cong
	% 		\left\{
	% 		\footnotesize
	% 		\begin{tikzcd}[ampersand replacement=\\&]
	% 			\mathcal C_{/c} \ar[rr,"\varphi"]\ar[dr, "U"] \&\& \mathcal E \ar[dl, "p"]\\
	% 			\& \mathcal B \&
	% 		\end{tikzcd}
	% 		\right\}
	% 	\]
	% 	for every discrete fibration $p : \mathcal E\to \mathcal B$.
